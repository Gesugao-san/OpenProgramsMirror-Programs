...
local function find_component(target_type, need_second)
  ...
  if need_second then
    ...
    if Targe1 ~= nil then print('Target2: Address: ' .. Target2.address .. '\nDebug: ' .. serialization.serialize(Target2)) end
  else
...
local function motion(address, relativeX, relativeY, relativeZ, entityName)
  ...
  local vectorLength = tonumber(string.format("%.3f", math.sqrt(math.abs(relativeX) + math.abs(relativeY) + math.abs(relativeZ))))
  ...
  print(address .. ' | ' .. x .. ' | ' .. y .. ' | ' .. z .. ' | ' .. entityName .. ' | ' .. vectorLength)
end
